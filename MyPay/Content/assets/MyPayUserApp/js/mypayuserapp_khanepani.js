...
$("#btnNextStep").click(function () {
    ...
    setTimeout(
        function () {
            ...
            $.ajax({
                type: "POST",
                url: "/MyPayUser/MyPayUserKhanePaniCounterDetails",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: false,
                data: '{"Counter":"' + Counters + '","CustomerID":"' + Number + '"}',
                success: function (response) {
                    ...
                    if (IsValidJson) {
                        ...
                        if (arr['Message'] == "success") {
                            $("#spnCounter").html($("#Counters").val());
                            $("#spnCustomerId").html($("#Number").val());
                            $("#spnCustomerName").html();
                            $("#DivKhanePaniStep2").show();
                            $("#DivKhanePani").hide();
                            $("#DivProceedToPay").hide();
                            $("#spnCustomerId").html(arr['Consumer_Code']);
                            $("#spnCustomerName").html(arr['Consumer_Name']);
                            $("#spnDueAmount").html(arr['Total_Dues']);
                            $("#Amount").val(arr['Total_Dues']);
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (arr['Message'] == "success") {
                        ...
                        $("#ConsumerIdPopup").text($("#Number").val());
...
function EnableDisablePayButton() {
    ...
    if ($("#Number").val().length > 0) {
        ...
        if ($("#Counters").val() != "undefined" && $("#Counters").val() != "0") {
...
function Pay() {
    ...
    if (Counter == "") {
        $("#dvMessage").html("Please enter Counter");
        return false;
    }
    ...
    if (PaymentMode == "0") {
        ...
        setTimeout(
            function () {
                $.ajax({
                    type: "POST",
                    url: "/MyPayUser/MyPayUserKhanePani",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: false,
                    data: '{"ServiceID":"' + ServiceID + '","Counter":"' + Counter + '","CustomerID":"' + CustomerID + '","Amount":"' + Amount + '","Mpin":"' + Mpin + '","PaymentMode":"' + PaymentMode + '","Couponcode":"' + Couponcode + '","BankId":"' + $("#hfBankId").val() + '"}',
                    success: function (response) {
