    ...
    setTimeout(
        function () {
            table = $('#tblist').DataTable({
                ...
                "order": [[1, "desc"]],
                "ajax": {
                    "url": "/BalanceHistoryReport/GetBalanceHistoryLists",
                    "type": "POST",
                    "async": false,
                    data: function (data) {
                        take = data.length;
                ...
                "columns": [
                    ...
                    { "data": "UserCount", "name": "User Count", "autoWidth": true, "bSortable": false },
                    { "data": "ActiveUser", "name": "Active User", "autoWidth": true, "bSortable": false },
                    { "data": "InActiveUser", "name": "InActive User", "autoWidth": true, "bSortable": false },
                    { "data": "TotalCoinsBalance", "name": "Total Coins Balance", "autoWidth": true, "bSortable": false },
                    {
                        data: null,
                        render: function (data, type, row) {
                            return '<a href="javascript:void(0);" onclick="return EditBalance(&apos;' + data.Id + '&apos;,&apos;' + data.TotalBalance + '&apos;,&apos;' + data.TypeName + '&apos;,&apos;' + data.UserCount + '&apos;,&apos;' + data.ActiveUser + '&apos;,&apos;' + data.InActiveUser + '&apos;,&apos;' + data.TotalCoinsBalance + '&apos;)"  class="btn btn-primary btn-sm btn-icon"><em class="icon ni ni-edit"></em></a>';
                        },
                        bSortable: false,
                        sTitle: "Edit"
                    }
...
function EditBalance(Id, TotalBalance, TypeName, UserCount, ActiveUser, InActiveUser, TotalCoinsBalance) {
    $('#AjaxLoader').show();
    setTimeout(
        function () {
            if (Id != "" || Id != null) {
                ...
                $("#spnUserCount").html(UserCount);
                $("#spnActiveUser").html(ActiveUser);
                $("#spnInActiveUser").html(InActiveUser);
                $("#spnTotalCoinsBalance").html(TotalCoinsBalance);
                $("#spnTotalCoinsBalance").html(TotalCoinsBalance);
                $("#hdnid").val(Id);
    ...
    setTimeout(
        function () {
            ...
            if (id == "" || id == "0") {
                ...
                $.ajax({
                    type: "POST",
                    url: "/BalanceHistoryReport/EditBalanceHistory",
