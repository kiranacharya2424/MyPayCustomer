...
{
    public class AddBalanceHistoryMerchant : CommonAdd
    {
        #region "enum"
        public enum Types
        {
        ...
        //UserCount 
        private Int64 _MerchantCount = 0;
        public Int64 MerchantCount
        {
        ...
        //ActiveUser 
        private Int64 _ActiveMerchant = 0;
        public Int64 ActiveMerchant
        {
            get { return _ActiveMerchant; }
            set { _ActiveMerchant = value; }
        }

        //InActiveUser 
        private Int64 _InActiveMerchant = 0;
        public Int64 InActiveMerchant
        {
            get { return _InActiveMerchant; }
            set { _InActiveMerchant = value; }
        }
        ...
        public static decimal GetTotalBalance()
        {
            ...
            try
            {
                ...
                string Result = "";
                string str = "Select isnull(sum(MerchantTotalAmount),0) from Merchant with(nolock)";
        ...
        public static Int64 GetTotalMerchant()
        {
            Int64 data = 0;
            try
            {
                CommonHelpers obj = new CommonHelpers();
                string Result = "";
                string str = "Select isnull(count(Id),0) from Merchant with(nolock)";
                Result = obj.GetScalarValueWithValue(str);
      ...
      /*  public static string DisconnectedServiceConnection()
        ...
        public static Int64 GetTotalActiveMerchant()
        {
            Int64 data = 0;
            try
            {
                CommonHelpers obj = new CommonHelpers();
                string Result = "";
                string str = "Select isnull(count(Id),0) from Merchant with(nolock) where IsActive=1";
                Result = obj.GetScalarValueWithValue(str);
                if (!string.IsNullOrEmpty(Result) && Result != "0")
                {
                    data = Convert.ToInt64(Result);
        ...
        public static Int64 GetTotalInActiveMerchant()
        {
            Int64 data = 0;
            try
            {
                CommonHelpers obj = new CommonHelpers();
                string Result = "";
                string str = "Select isnull(count(Id),0) from Merchant with(nolock) where IsActive=0";
                Result = obj.GetScalarValueWithValue(str);
                if (!string.IsNullOrEmpty(Result) && Result != "0")
                {
                    data = Convert.ToInt64(Result);
