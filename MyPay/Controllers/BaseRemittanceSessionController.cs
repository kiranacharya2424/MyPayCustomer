...
{
    public class BaseRemittanceSessionController : Controller
    {
        protected override void OnActionExecuted(ActionExecutedContext filterContext)
        {
            ...
            try
            {
                ...
                if (!isAjax)
                {
                    if (Session["RemittanceRole"] != null && Session["RemittanceRole"].ToString() != "0" && Session["RemittanceRole"].ToString() != "1")
                    {
                        string currenturl = Request.Url.AbsolutePath;
                        var RoleId = int.Parse(Session["RemittanceRole"].ToString());
                    ...
                    else if (Session["RemittanceRole"] == null || Session["RemittanceRole"].ToString() == "0" || Session["RemittanceUniqueId"] == null)
                    {
                        Response.Redirect("/RemittanceLogin");
                    }
                    AddMerchant outobject = new AddMerchant();
                    GetMerchant inobject = new GetMerchant();
                    inobject.MerchantUniqueId = Convert.ToString(Session["RemittanceUniqueId"]);
                    ...
                    if (model != null && model.Id != 0)
                    {
                        ...
                        {
                            Response.Redirect("/RemittanceLogin/ChangePassword");
                        }
                    }
                }
