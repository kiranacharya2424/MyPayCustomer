...
//using ClosedXML.Excel.XLPredefinedFormat;
...
namespace MyPay.Controllers
{
    ...
    public class FileUploadController : Controller
    {
        ...
        //[System.Web.Http.Route("api/v2/uploadFiles2")]
        [System.Web.Http.HttpPost]
        public void Index()
        {
                    ...
                    if (Request.Files.Count > 0)
                    {
                                ...
                                if (file.ContentLength < TenMegaBytes)
                                {
                                    if (!string.IsNullOrEmpty(fileName))
                                    {
                                        ...
                                        if (file_ext == ".jpeg" || file_ext == ".jpg" || file_ext == ".png" || file_ext == ".gif" || file_ext == ".bmp")
                                        {
                                            ...
                                            var path = Server.MapPath("~/UserDocuments/Images/");
                                            file.SaveAs(path + uploadfilename);


                                            using (Image image = Image.FromFile(Server.MapPath("~/UserDocuments/Images/") + uploadfilename))
                                            {
                                                using (MemoryStream m = new MemoryStream())
                                                {
                                                    try
                                                    {
                                                        ...
                                                        else if (Request.Form["Type"].ToString().ToLower() == "selfie")
                                                        {
                                                            ...
                                                            // Session["MyPayUserImage"] = "/UserDocuments/Images/" + uploadfilename;
                                                            Session["MyPayUserImage"] = "/UserDocuments/Images/" + uploadfilename;
                        ...
                        if (checkflag)
                        {
                            ...
                            if (Id > 0 || updatestatus)
                            {
                                ...
                                if (status)
                                {
                                    //Session["IsKycApproved"] = res.IsKYCApproved;
                                    //Session["DocumentReason"] = res.DocumentReason;
                                    Response.StatusCode = (int)HttpStatusCode.OK;
                        ...
                        if (!string.IsNullOrEmpty(fileName))
                        {
                            ...
                            if (file_ext == ".jpeg" || file_ext == ".jpg" || file_ext == ".png" || file_ext == ".gif" || file_ext == ".bmp" || foldername == "TicketsImages")
                            {
                                if (foldername == "UploadReceipt")
                                {
                                    ...
                                    //    if (w.Update())
                                    //    {
                                    //        if (File.Exists(Server.MapPath("~/Admin/Images/" + foldername + "/") + w.UploadReceipt))
                                    //        {
                                    //            File.Delete(Server.MapPath("~/Admin/Images/" + foldername + "/") + w.UploadReceipt);
                                    //        }
                                    //        file.SaveAs(Server.MapPath("~/Admin/Images/" + foldername + "/") + w.UploadReceipt);
                                    //        context.Response.StatusCode = (int)HttpStatusCode.OK;
                                ...
                                else
                                {
                                    ...
                                    //file.SaveAs(Server.MapPath("~/Images/" + foldername + "/") + uploadfilename);
                                    file.SaveAs(Server.MapPath("~/Images/" + foldername + "/") + uploadfilename);
                                    Response.StatusCode = (int)HttpStatusCode.OK;
