...
namespace MyPay.API.Controllers
{
    ...
    {
        private static ILog log = LogManager.GetLogger(typeof(TicketReplyController));
        public HttpResponseMessage Post(Req_TicketReply user)
        {
            log.Info($"{System.DateTime.Now.ToString()} inside TicketReplyController" + Environment.NewLine);
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(user.MemberId.ToString()) && user.MemberId.ToString() != "0")
                    {
                        ...
                        int VendorAPIType = 0;
                        int Type = 0;
                        resUser = new CommonHelpers().CheckUserDetail(user.UniqueCustomerId, userInput, "", user.BankTransactionId, ref resGetCouponsScratched, user.CouponCode, user.DeviceId, ref CommonResult, Type, VendorAPIType, memId, false, false, "0", false, user.Mpin, "", true, true);
                    ...
                    string msg = RepTicket.TicketReplyDetail(authenticationToken, user.Take, user.Skip, user.Description, resUser, user.MemberId, user.TicketId, user.Name, user.Image, user.PlatForm, true, user.DeviceCode);
                    ...
                    else
                    {
                        ...
                        log.Info($"{System.DateTime.Now.ToString()} TicketReplyController completed" + Environment.NewLine);
                        return response;
                    }
                }
            }
